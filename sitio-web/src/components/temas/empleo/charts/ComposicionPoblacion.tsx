...
import { COLORS, formatFull } from "../../../charts/shared";
...
export default function ComposicionPoblacion() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/data/empleo/poblacion_colombia_2018_2070.csv").then((r) => r.text()),
      fetch("/data/empleo/geih_total_nacional_mensual_2001_2026.csv").then((r) => r.text()),
    ]).then(([popCsv, geihCsv]) => {
      // Total 2026
